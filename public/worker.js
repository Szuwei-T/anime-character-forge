...
async function assets(req, env) {
  const db = env.DB;
  const rows = await db.prepare("SELECT assetId, type, rarity, imageUrl FROM assets ORDER BY type, rarity, assetId").all();
...
async function pickAssetWithFeaturedBoost(db, whereSql, binds, featuredIds, boost = 3) {
  ...
  const baseSelect = `SELECT assetId, type, rarity, imageUrl FROM assets WHERE ${whereSql}`;
...
async function apiRecipes(req, env) {
  ...
  const rows = await db.prepare(`SELECT ${recipeCol} AS recipeId FROM recipes ORDER BY ${recipeCol}`).all();
...
async function seed(req, env) {
  ...
  const stmt = db.prepare("INSERT INTO assets (assetId, type, rarity, imageUrl) VALUES (?1, ?2, ?3, ?4)");
  for (const a of assets) await stmt.bind(a.assetId, a.type, a.rarity, a.imageUrl).run();
...
async function apiMeSaves(req, env) {
  ...
  try {
    const rows = await db
      .prepare("SELECT userId, headId, bodyId, bgId, addon1Id, addon2Id, savedOn FROM user_saves WHERE userId=?1 ORDER BY savedOn DESC")
      .bind(userId)
      .all();
...
async function gacha(req, env) {
  ...
  // ===== Costs =====
  // Normal: 100 gold per roll (10-roll = 1000)
  // Premium: 10 gem per roll (10-roll = 100)
  const costGold = premium ? 0 : (100 * count);
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (!a) return json({ ok:false, error:"no_asset_found", type: t }, 500);
...
async function save(req, env) {
  ...
  if (!alreadySaved) {
    const sql =
      "INSERT INTO user_saves (userId, headId, bodyId, bgId, addon1Id, addon2Id, savedOn) " +
      "VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)";
    try {
      await db.prepare(sql).bind(userId, headId, bodyId, bgId, addon1Id, addon2Id, savedOn).run();
    } catch (e) {
      writeError = String(e?.message || e);
    }
  }
...
async function gallery(req, env) {
  ...
  const sql = `
      SELECT
        ...
        (SELECT COUNT(*) FROM recommends_v1 r WHERE r.seasonId = s.seasonId AND r.showcaseId = s.showcaseId) as recommends,
...
async function apiRecommend(req, env){
  ...
  // per-user per-season per-showcase cap
  const usedRow = await db.prepare("SELECT COUNT(*) AS c FROM recommends_v1 WHERE seasonId=?1 AND showcaseId=?2 AND recommenderId=?3")
    .bind(seasonId, showcaseId, userId).first();
  const used = Number(usedRow?.c || 0);
  if (used >= 5) {
    const accX = await db.prepare("SELECT userGem FROM user_accounts WHERE userId=?1").bind(userId).first();
    return json({ ok:false, error:"max_per_showcase_reached", max:5, used, costGem:50, userGem:Number(accX?.userGem||0), seasonId }, 400);
  }
  ...
  if (curGem < 50) return json({ ok:false, error:"insufficient_gem", costGem:50, userGem:curGem, seasonId }, 400);

  // atomic spend
  const dec = await db.prepare("UPDATE user_accounts SET userGem = userGem - 50 WHERE userId=?1 AND userGem >= 50").bind(userId).run();
  if ((dec?.meta?.changes || 0) <= 0) {
    const acc2 = await db.prepare("SELECT userGem FROM user_accounts WHERE userId=?1").bind(userId).first();
    return json({ ok:false, error:"insufficient_gem", costGem:50, userGem:Number(acc2?.userGem||0), seasonId }, 400);
  }

  await db.prepare("INSERT INTO recommends_v1 (seasonId, showcaseId, recommenderId, createdAt) VALUES (?1,?2,?3,?4)")
    .bind(seasonId, showcaseId, userId, Date.now()).run();

  const rc = await db.prepare("SELECT COUNT(*) AS c FROM recommends_v1 WHERE seasonId=?1 AND showcaseId=?2").bind(seasonId, showcaseId).first();
  const recommendCount = Number(rc?.c || 0);
  ...
  return json({ ok:true, seasonId, recommends: recommendCount, recommendCount, used: used + 1, maxPerShowcase:5, costGem:50, userGem:Number(acc3?.userGem||0) });
}
...
async function apiRankNewcomers(req, env){
  ...
  const sql = `
    SELECT
      ...
      ua.userName as authorName,
      ua.level as authorLevel,
      ua.createdAt as authorCreatedAt,
      us.headId as authorAvatarHeadId,
      us.bodyId as authorAvatarBodyId,
      us.bgId as authorAvatarBgId,
      us.addon1Id as authorAvatarAddon1Id,
      ...
      aa2.imageUrl as addon2Url
    FROM showcases s
    JOIN user_accounts ua ON ua.userId = s.userId
    LEFT JOIN user_saves us ON us.userId = s.userId AND us.savedOn = ua.avatarSaveOn
    LEFT JOIN assets ah ON ah.assetId = s.headId
    LEFT JOIN assets ab ON ab.assetId = s.bodyId
    LEFT JOIN assets ag ON ag.assetId = s.bgId
...
async function apiMeAccount(req, env) {
  ...
  if (avatarSaveOn) {
    const srow = await db
      .prepare("SELECT userId, headId, bodyId, bgId, addon1Id, addon2Id, savedOn FROM user_saves WHERE userId=?1 AND savedOn=?2")
